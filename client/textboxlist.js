/* This is TextBoxList by Guillermo Ranch http://www.devthought.com/projects/jquery/textboxlist/ */
/* Sources extracted from demo due to non-existing download page */
/* Also, incorporated bugfixes from comments (added with AAS comments) */
...
$.TextboxList = function(element, _options){
    ...
    var options = $.extend(true, {
        ...
        encode: function(o){ 
            return $.grep($.map(o, function(v){     
                v = (chk(v[0]) ? v[0] : v[1]);
                return chk(v) ? v.toString().replace(/,/g, '') : null; // AAS: changed from /,/ (added global flag)
            }), function(o){ return o != undefined; }).join(','); 
        },
        decode: function(o){ return o.split(/\s*,\s*/); } // AAS: changed from ',' to get rid of spaces.
    }, _options);
    ...
    var create = function(klass, value, opt){
        if (klass == 'box'){
            ...
            if ($.trim(value[1]) === "") return false; // AAS: Added: dont allow empty values.
        }       
    ...
    var update = function(){
        ...
        original.change(); // AAS: added to make events possible.
    };
...
$.TextboxListBit = function(type, value, textboxlist, _options){
    ...
    var options = $.extend(true, type == 'box' ? {
            ...
            addOnBlur: false,
            addKeys: [13],
            addDecoded: false // AAS: Add bits according to options.decoded fn.
        }, _options);
    ...
    var init = function(){
        ...
        if (type == 'box'){
            ...
            // AAS: Add bits according to options.decode fn.
            if (options.addDecoded){
                element.keyup(function(ev){
                    if (!focused) return;
                    var parts = textboxlist.getOptions().decode(self.getValue()[1]);
                    if (parts.length > 1) {
                        while (parts.length > 1){
                            var value = [null, parts.shift(), null];
                            var b = textboxlist.create('box', value);
                            if (b){
                                b.inject(bit, 'before');
                            }
                        }
                        self.setValue([null, parts[0], null]);
                    }
                });
            }
        }
...
$.fn.extend({
    ...
    textboxlist: function(options){
        ...
        return this.each(function(){
            var data = $(this).data('textboxlist')
              , optionsObj = typeof options === 'object' ? options : {}

            if (!data) $(this).data('textboxlist', (data = new $.TextboxList(this, optionsObj)))
            
            if (typeof options == 'string') data[options].apply(data, args.slice(1));
        });
    }
...
$.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
    ...
    var search = function(bit){
        ...
        container.width(textboxlist.getContainer().width()); // AAS: Width is unknown on init, so set it here.
...
/* Placeholder plugin. Text is taken from 'placeholder' attr of the input. (c) AAS */

(function(){
    
$.TextboxList.Placeholder = function(textboxlist, _options){
    
    var prefix, element, original;

    var init = function(){
        textboxlist
            .addEvent('blur', update)
            .addEvent('focus', update)
            .addEvent('bitAdd', update);
        
        prefix = textboxlist.getOptions().prefix + '-placeholder';
        original = textboxlist.getContainer().prev()
        placeholderText = original.attr('placeholder');
        if (!placeholderText) return;
        element = $('<div/>')
            .addClass(prefix)
            .text(placeholderText)
            .click(function() {textboxlist.getContainer().click(); }) // Pass through clicks.
            .prependTo(textboxlist.getContainer());
        update();
    };

    var update = function(){
        var focused = textboxlist.getContainer().is('.' + textboxlist.getOptions().prefix + '-focus');
        var hasBoxes = textboxlist.getContainer().find(".textboxlist-bit-box").length > 0;
        var hasFilledInputs = textboxlist.getContainer()
            .find(".textboxlist-bit-editable-input")
            .filter(function(){return $(this).val() != ""}).length > 0;
        
        element.toggle(!focused && !hasBoxes && !hasFilledInputs);
    };
